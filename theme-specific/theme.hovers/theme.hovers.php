...
if (!function_exists('electroserv_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'electroserv_hovers_theme_setup3', 3 );
	function electroserv_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		electroserv_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'electroserv'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'electroserv') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'electroserv'),
					'slide_left'	=> esc_html__('Slide from Left',	'electroserv'),
					'slide_right'	=> esc_html__('Slide from Right',	'electroserv'),
					'slide_top'		=> esc_html__('Slide from Top',		'electroserv'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'electroserv'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'electroserv'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'electroserv') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'electroserv'),
					'icon'	=> esc_html__('Icon',	'electroserv'),
					'icons'	=> esc_html__('Icons',	'electroserv'),
					'zoom'	=> esc_html__('Zoom',	'electroserv'),
					'fade'	=> esc_html__('Fade',	'electroserv'),
					'slide'	=> esc_html__('Slide',	'electroserv'),
					'pull'	=> esc_html__('Pull',	'electroserv'),
					'border'=> esc_html__('Border',	'electroserv')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('electroserv_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'electroserv_hovers_theme_setup9', 9 );
	function electroserv_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'electroserv_hovers_frontend_scripts', 1010 );
		add_filter( 'electroserv_filter_localize_script','electroserv_hovers_localize_script' );
		add_filter( 'electroserv_filter_merge_scripts',	'electroserv_hovers_merge_scripts' );
		add_filter( 'electroserv_filter_merge_styles',	'electroserv_hovers_merge_styles' );
		add_filter( 'electroserv_filter_get_css', 		'electroserv_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'electroserv_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'electroserv_hovers_frontend_scripts', 1010 );
	function electroserv_hovers_frontend_scripts() {
		if ( electroserv_is_on(electroserv_get_theme_option('debug_mode')) && electroserv_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'electroserv-hovers', electroserv_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( electroserv_is_on(electroserv_get_theme_option('debug_mode')) && electroserv_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'electroserv-hovers',  electroserv_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('electroserv_hovers_merge_scripts')) {
	//Handler of the add_filter( 'electroserv_filter_merge_scripts', 'electroserv_hovers_merge_scripts' );
	function electroserv_hovers_merge_scripts($list) {
...
if (!function_exists('electroserv_hovers_merge_styles')) {
	//Handler of the add_filter( 'electroserv_filter_merge_styles', 'electroserv_hovers_merge_styles' );
	function electroserv_hovers_merge_styles($list) {
...
if (!function_exists('electroserv_hovers_localize_script')) {
	//Handler of the add_filter( 'electroserv_filter_localize_script','electroserv_hovers_localize_script' );
	function electroserv_hovers_localize_script($arr) {
		$arr['button_hover'] = electroserv_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('electroserv_hovers_add_icons') ) {
	function electroserv_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'electroserv') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'electroserv');
						else
							esc_html_e('Details', 'electroserv');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-link-button"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'electroserv_hovers_get_css' ) ) {
	//Handler of the add_filter( 'electroserv_filter_get_css', 'electroserv_hovers_get_css', 10, 4 );
	function electroserv_hovers_get_css($css, $colors, $fonts, $scheme='') {
