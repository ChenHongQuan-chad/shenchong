...
if ( has_post_thumbnail() ) {
	$image = trx_addons_get_attachment_url( get_post_thumbnail_id( get_the_ID() ), trx_addons_get_thumb_size('chess') );
}
...
?>>
	<div class="sc_services_item_header <?php echo esc_attr($args['hide_bg_image']==1 ? ' without_image' : ''); ?>"<?php if ($args['hide_bg_image']==0 && !empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
		<div class="sc_services_item_header_inner">
			...
			if ($args['featured']=='icon' && !empty($meta['icon'])) {
				...
				?>><?php
					if (!empty($svg)) {
						...
						?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="'.esc_attr__('icon_image', 'electroserv').'"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
					}
				?></a><?php
			...
			?>
			<h2 class="sc_services_item_title"><a href="<?php echo esc_url($link); ?>"><?php the_title(); ?></a></h2>
	...
	<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
		<div class="sc_services_item_content"<?php if (!empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
			<div class="sc_services_item_content_inner">
				<h2 class="sc_services_item_title"><a href="<?php echo esc_url($link); ?>"><?php the_title(); ?></a></h2>
                <a class="sc_button" href="<?php echo esc_url($link); ?>"><?php esc_html_e('Read More', 'electroserv'); ?></a>
            </div>
		</div>
	<?php } ?>
</div>
