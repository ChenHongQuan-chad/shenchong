	...
	foreach ($args['prices'] as $item) {
		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				?><div class="sc_price_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="'.esc_attr__('image', 'electroserv').'"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
			} else {
				 ...
				 if (!empty($icon)) {
					...
					?><div class="sc_price_item_icon sc_price_item_icon_type_<?php echo esc_attr($item['icon_type']); ?> <?php echo esc_attr($icon); ?>"
						...
						if (!empty($img)) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="'.esc_attr__('icon', 'electroserv').'"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						} else {
